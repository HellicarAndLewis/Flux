...
//  FluxOF
//
//  Created by Patricio Gonzalez Vivo on 6/13/14.
//
//

#include "ColorPalette.h"

float ColorTheory::luminance(ofColor col) {
    return col.r * 0.299f *255+ col.g * 0.587f *255+ col.b * 0.114f*255;
}

ofColor ColorTheory::getDarkest(vector<ofColor> &colors) {
    ofColor darkest;
...
float ColorTheory::distanceBetween(ofColor a,ofColor b) {
...
void ColorTheory::sortByDistance(vector<ofColor> &colors) {
    if (colors.size() == 0) {
        return;
    }
...
vector<ofVec2f> ColorTheory::getRYB_WHEEL(){
...
ofColor ColorTheory::rotateRYB(ofColor col,int theta) {
...
ofColor ColorTheory::getComplement(ofColor col){
    return rotateRYB(col,180);
};

vector<ofColor> ColorTheory::getColorPalette(ofImage &_img, int _nColors){
